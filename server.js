  ...
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.match(/\d{1,2}\s+(janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+\d{4}/i)) {
        ...
        // Chercher les 5 numéros + 1 numéro chance dans les lignes suivantes
        let nums = [];
        let chance = 0;
        ...
        for (let j = i + 1; j < Math.min(i + 10, lines.length); j++) {
          ...
          if (matches && matches.length >= 5) {
            nums = matches.slice(0, 5).map(Number);
            chance = parseInt(matches[5]) || 0;
    ...
    if (results.length > 0) {
      ...
      console.log(`✅ ${results.length} résultats FDJ scrapés depuis magayo.com`);
      return results;
    }
