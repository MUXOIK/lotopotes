...
async function chargerDonnees() {
  ...
  try {
    ...
    if (res.ok && res.data.content) {
      ...
      PARTICIPANTS.forEach(p => { distribution[p]={gains:0,solde:-180}; });
      console.log('[DB] Fichier vide/inexistant');
    }
  } catch(e) {
    PARTICIPANTS.forEach(p => { distribution[p]={gains:0,solde:-180}; });
...
async function sauvegarder() {
  ...
  try {
    ...
    const res = await githubRequest('PUT', 'data.json', {
      message: 'data ' + new Date().toISOString().split('T')[0],
      content,
      ...(dataFileSha ? {sha: dataFileSha} : {})
    });
    if (res.ok) { dataFileSha = res.data.content.sha; console.log('[DB] ✅ Sauvegardé'); }
    else console.log('[DB] Erreur save: ' + JSON.stringify(res.data));
...
function fetch2(hostname, path) {
  return new Promise((resolve) => {
    const options = {
      hostname, path, method: 'GET',
      headers: {'User-Agent':'Mozilla/5.0 Chrome/120','Accept':'text/html','Accept-Language':'fr-FR','Cache-Control':'no-cache'},
      timeout: 20000
    };
    const req = https.request(options, (res) => {
      let data = '';
...
function parseMontants2nd(html) {
  // Chercher le bloc après "tabpanel-1" ou "Option 2nd"
  const rg = {};
  // Trouver le début du 2nd tirage tableau
  ...
  console.log('[SCRAPE] Montants 2nd: 5=' + rg['5'] + '€, 4=' + rg['4'] + '€, 3=' + rg['3'] + '€, 2=' + rg['2'] + '€');
  return rg;
}

async function scraper() {
  console.log('[SCRAPE] Démarrage...');
  const main = await fetch2('www.secretsdujeu.com', '/page/jeux_loto_resultats.html');
  ...
  console.log('[SCRAPE] 1er: ' + nums.join(',') + ' C' + chance);

  // PAGE DÉTAIL
  const urlM = /"url":"(https:\/\/www\.secretsdujeu\.com\/loto\/resultat\/tirage-loto-du-[^"]+)"/.exec(html);
  let rg1 = null, rg2 = {}, nums2 = [];

  if (urlM) {
    const path = urlM[1].replace('https://www.secretsdujeu.com','');
    const detail = await fetch2('www.secretsdujeu.com', path);
    if (detail.ok && detail.status === 200) {
      ...
      console.log('[SCRAPE] Montants 1er: 5=' + rg1['5'] + '€, 4+1=' + rg1['4+1'] + '€, 1+1=' + rg1['1+1'] + '€');
    }
  }

  if (!rg1 || Object.values(rg1).every(v=>v===0))
    rg1 = {'5+1':0,'5':100000,'4+1':1000,'4':500,'3+1':50,'3':20,'2+1':9,'2':4,'1+1':2.20};

  tirageCache = {nums, chance, nums2, date, rapportGains: rg1, rapportGains2: rg2};
  cacheExpiry = prochainTirage();
  console.log('[SCRAPE] ✅ Cache valide jusqu\'au ' + cacheExpiry);
  return tirageCache;
}
...
function gain1er(grille, nums, cg, ct, rg) {
...
function gain2nd(grille, nums2, rg2) {
  const n = nums2.filter(x=>grille.includes(x)).length;
  if (n===5) return rg2['5']||0; if (n===4) return rg2['4']||0;
  if (n===3) return rg2['3']||0; if (n===2) return rg2['2']||0;
  return 0;
}
...
app.get('/api/loto-complet', async (req, res) => {
  try {
    ...
    if (!dejàEnregistré(tirage)) {
      ...
      const a2 = tirage.nums2 && tirage.nums2.length === 5;

      GRILLES.forEach((g,i) => {
        const g1 = gain1er(g, tirage.nums, CHANCES[i], tirage.chance, tirage.rapportGains);
        const g2 = a2 ? gain2nd(g, tirage.nums2, rg2) : 0;
        if (g1>0) console.log('[LOG] 🎉 1er Grille'+(i+1)+': +'+g1+'€');
        if (g2>0) console.log('[LOG] 🎉 2nd Grille'+(i+1)+': +'+g2+'€');
        total += g1 + g2;
      });

      console.log('[LOG] Total gains: ' + total + '€');
      cagnotte += total;
      if (cagnotte>=650) {
        const pp=650/13;
      ...
      allTirages.push({nums:tirage.nums, chance:tirage.chance, nums2:tirage.nums2, gains:total, rapportGains:tirage.rapportGains, rapportGains2:rg2, date:tirage.date});
      await sauvegarder();
    }
...
app.get('/api/force-scrape', async (req, res) => {
  console.log('[ADMIN] Force scrape + reload');
  tirageCache = null; cacheExpiry = null;
  await chargerDonnees();
  const tirage = await scraper();
  if (!tirage) return res.json({success:false, error:lastError});
  res.json({success:true, tirage, message:'Cache vidé. Allez sur Accueil pour recalculer.', timestamp:new Date().toISOString()});
});
