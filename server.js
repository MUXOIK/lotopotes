...
async function scraper() {
  ...
  // Réhydrater rapportGains2 depuis allTirages si le scraping ne l'a pas obtenu
  if (!rg2 || Object.values(rg2).every(v=>v===0)) {
    const dateStr = date.split('T')[0];
    const existant = allTirages.find(t => t.date.split('T')[0] === dateStr);
    if (existant && existant.rapportGains2 && Object.values(existant.rapportGains2).some(v=>v>0)) {
      tirageCache.rapportGains2 = existant.rapportGains2;
      if (!tirageCache.nums2 || tirageCache.nums2.length === 0) tirageCache.nums2 = existant.nums2 || [];
      console.log('[SCRAPE] rapportGains2 réhydraté depuis historique');
    }
  }
