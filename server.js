...
let tirageScrape = null;
let cacheExpiry = null;
let dataHistoriqueSha = null;
let dataScrapeJeSha = null;
let allGains = [];
let distribution = {};
let cagnotte = 0;
let nombreTirages = 9;
let dataCountTiragesSha = null;
...
function parseMontants2nd(html) {
  ...
  for (let i = 0; i < lignes.length; i++) {
    ...
    if (cle in rg && rg[cle] === 0) { rg[cle] = montant !== null ? montant : 0; }
  }
  return rg;
}
...
app.get('/api/loto-complet', async (req, res) => {
  ...
  if (total > 0) { await ajouterAuHistorique(tirage, gainsDetails); }
